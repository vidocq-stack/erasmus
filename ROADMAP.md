...
| Integration tests | `CascadingAndGroupsTest`: nested cascading with dotted paths, null/absent-`@Valid` non-cascading, a circular two-node graph, a self-referencing node, `Default` vs. explicit group, group inheritance, and both directions of `@GroupSequence` short-circuiting (fails at first group / passes through to the second). 10 tests. | ✅ |
...
case. 68 tests total (up from 58), all green.
